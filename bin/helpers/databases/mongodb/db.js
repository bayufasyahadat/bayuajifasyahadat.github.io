...
class DB {
  ...
  async updateOne(parameter, updateQuery) {
    ...
    try {
      ...
      const connection = cacheConnection.db(dbName);
      const db = connection.collection(this.collectionName);
      const data = await db.updateOne(parameter, { $set: updateQuery });
      if (data.result.nModified >= 0) {
        const { result: { nModified } } = data;
        const recordset = await this.findOne(parameter);
        if (nModified === 0) {
  ...
  async customQuery(ctx) {
    ...
    try {
      ...
      logger.log(ctx, err.message, `Error ${ctx} data in mongodb`);
      return wrapper.error(`Error ${ctx} Mongo ${err.message}`);
    }
