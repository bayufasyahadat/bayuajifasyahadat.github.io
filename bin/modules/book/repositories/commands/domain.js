...
class Book {
  ...
  async register(payload) {
    const {nama, username, password, isActive } = payload;
    const user = await this.query.findOneUser({ username });
  ...
  async insert(payload) {
    const { data:result } = await this.command.insertOneBook(payload);
    return wrapper.data(result);
  }

  async update(bookId, payload){
    const {data:result} = await this.command.updateOneBook(bookId, payload);
    return wrapper.data(result);
  }

  async del(bookId, payload){
    const {data:result} = await this.command.deleteOneBook(bookId, payload);
    return wrapper.data(result);
  }
}
